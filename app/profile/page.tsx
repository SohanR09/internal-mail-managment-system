...
export const metadata = {
  title: "Profile | Northstar Mail",
  description: "Manage your Northstar Mail profile",
};
...
export default async function ProfilePage() {
  ...
  const defaultFolder =
    settings?.defaultFolder === "draft"
      ? "drafts"
      : settings?.defaultFolder || "inbox";
  ...
  return (
    <main className="flex h-full min-h-0 flex-col overflow-y-auto bg-background text-foreground">
      <PageHeader title="Profile" backHref={`/mail/${defaultFolder}`} />
      <ProfileForm
        initialProfile={{
