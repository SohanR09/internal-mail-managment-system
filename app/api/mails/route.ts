...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mail: Mail = { id: input.draftId ?? randomUUID(), threadId: input.threadId ?? (input.draftId ?? randomUUID()), parentMailId: input.parentMailId ?? null, senderId: sender.id, to: input.to, cc: input.cc, bcc: input.bcc, subject: input.subject.trim() || '(no subject)', bodyHtml, bodyText: `${input.bodyText.replace(/\\\\n/g, '\\n')}${signature ? `\\n\\n${signature}` : ''}`, isDraft: false, sentAt: now, createdAt: now };
