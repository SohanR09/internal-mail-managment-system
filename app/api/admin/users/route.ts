...
async function serializeUsers() {
  ...
  return users.map(({ passwordHash: _passwordHash, ...user }) => ({ ...user, roles: assignments.filter((a) => a.userId === user.id).map((a) => roles.find((r) => r.id === a.roleId)?.name).filter(Boolean) }));
}

export async function GET() {
  try { await requireRole('admin'); return NextResponse.json({ users: await serializeUsers(), roles: await db.getAllRoles() }); }
