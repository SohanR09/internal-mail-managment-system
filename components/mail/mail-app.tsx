...
export function MailApp({
  ...
  return (
    <main className="mail-shell grid h-dvh min-h-0 grid-cols-[auto_minmax(0,1fr)] grid-rows-[auto_minmax(0,1fr)] overflow-hidden bg-background text-foreground">
      ...
      {/* <aside
        className={`row-start-2 min-h-0 min-w-0 flex h-full shrink-0 flex-col overflow-y-auto border-r border-border p-3 transition-[width] duration-200 ${sidebarCollapsed ? "w-16" : "w-60"}`}
      >
        <div
          className={`mb-6 flex items-center gap-2 px-2 ${sidebarCollapsed ? "justify-center" : ""}`}
        >
          <span className="size-2 shrink-0 rounded-full bg-primary" />
          {!sidebarCollapsed ? (
            <span className="font-semibold tracking-tight">Northstar Mail</span>
          ) : null}
          <Button
            type="button"
            variant="ghost"
        ...
        <button
          type="button"
          aria-label={sidebarCollapsed ? "Compose message" : "Compose"}
          ...
          className={`mb-4 rounded-md bg-primary py-2 text-sm font-medium text-primary-foreground ${sidebarCollapsed ? "px-0" : "px-4"}`}
        >
          {sidebarCollapsed ? "+" : "Compose"}
        </button>
        <nav className="mb-4 flex flex-col gap-1" aria-label="Navigation">
          <button
            type="button"
            ...
            className={`sidebar-item flex items-center rounded-md px-3 py-2 text-left text-sm ${sidebarCollapsed ? "justify-center" : ""} text-muted-foreground hover:bg-accent/60`}
          >
          ...
          {session?.roles.includes("admin") ? (
            <button
              type="button"
              title={sidebarCollapsed ? "Admin users" : undefined}
              onClick={() => {
            ...
            >
              ...
              {sidebarCollapsed ? "A" : "Admin users"}
            </button>
          ) : null}
        </nav>
        <nav className="flex flex-col gap-1" aria-label="Mail folders">
          {folders.map((name) => (
            <button
              key={name}
              type="button"
              title={sidebarCollapsed ? labels[name] : undefined}
              onClick={() => {
                setSelected([]);
                router.push(`/mail/${name}`);
              }}
              className={`flex items-center justify-between rounded-md px-3 py-2 text-left text-sm ${sidebarCollapsed ? "justify-center" : ""} ${folder === name ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-accent/60"}`}
            >
              <span className={sidebarCollapsed ? "sr-only" : ""}>
                {labels[name]}
              </span>
        ...
        {!sidebarCollapsed ? (
          <div className="mt-auto rounded-lg border border-border bg-muted/30 p-3">
            <p className="text-xs font-medium">Team space</p>
        ...
        <button
          ...
          title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}
          className="mt-3 rounded-md px-3 py-2 text-sm text-muted-foreground hover:bg-accent"
        >
          {sidebarCollapsed ? "›" : "‹"}
        </button>
      </aside> */}
            ...
            <header className="flex items-center gap-4 border-b border-border px-6 py-4">
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-border px-6 py-3">
                <button
                  ...
                  className={`rounded-md border px-3 py-1 text-xs ${filters.unread ? "bg-accent" : ""}`}
                >
                  Unread
                </button>
                <button
                  ...
                  className={`rounded-md border px-3 py-1 text-xs ${filters.starred ? "bg-accent" : ""}`}
                >
                  Starred
                </button>
                <input
              ...
              <div className="ml-auto flex shrink-0 items-center gap-2">
                <button
                  ...
                  className="rounded-md px-2 py-2 text-xs text-muted-foreground hover:bg-accent"
                >
                  Refresh
                </button>
                <button
