...
export function ComposeWindow({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "min-h-48 p-3 outline-none prose prose-invert max-w-none",
      },
    },
  });

  useEffect(() => {
    if (editor && initialBodyHtml) editor.commands.setContent(initialBodyHtml);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      const response = await fetch(
        `/api/users/search?q=${encodeURIComponent(recipientInput)}`,
        { signal: controller.signal },
      );
      if (response.ok) {
        const result = (await response.json()) as { users: UserResult[] };
  ...
  async function saveDraft(): Promise<string | undefined> {
    ...
    const response = await fetch(
      draftKey ? `/api/mails/draft/${draftKey}` : "/api/mails/draft",
      {
        method: draftKey ? "PATCH" : "POST",
    ...
    if (!draftKey) {
      const result = (await response.json()) as { mail: { id: string } };
      setDraftKey(result.mail.id);
  ...
  async function send() {
    ...
    if (!response.ok) {
      ...
      setError(result.error ?? "Unable to send mail");
      return;
    }
    onClose();
  }
  function addRecipient(user: Recipient) {
    const setter =
      activeField === "to" ? setTo : activeField === "cc" ? setCc : setBcc;
    setter((current) =>
  ...
  function removeRecipient(email: string) {
    const setter =
      activeField === "to" ? setTo : activeField === "cc" ? setCc : setBcc;
    setter((current) => current.filter((item) => item.email !== email));
  }
  const current = activeRecipients[activeField];
  return (
    <div className="fixed bottom-4 right-4 z-20 w-[min(640px,calc(100vw-2rem))] overflow-hidden rounded-lg border border-border bg-card shadow-2xl">
      ...
      <div className="flex flex-col gap-2 p-4">
        <RecipientField
          ...
          input={activeField === "to" ? recipientInput : ""}
          onInput={setRecipientInput}
        />
        ...
        <input
          ...
          placeholder="Subject"
          className="border-b border-border bg-transparent px-1 py-2 text-sm outline-none"
        />
        <div className="flex flex-wrap gap-1 border-b border-border pb-2">
          ...
          <button
            ...
            className="rounded px-2 py-1 text-xs italic hover:bg-accent"
          >
            I
          </button>
          <button
            ...
            className="rounded px-2 py-1 text-xs underline hover:bg-accent"
          >
            U
          </button>
        </div>
...
function RecipientField({
  ...
  return (
    <div className="flex min-h-10 flex-wrap items-center gap-1 border-b border-border">
      ...
      <input
        ...
        placeholder={recipients.length ? "" : "Search people"}
        className="min-w-24 flex-1 bg-transparent px-1 py-2 text-sm outline-none"
      />
    </div>
  );
}
